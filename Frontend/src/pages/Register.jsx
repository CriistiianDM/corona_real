...
const Register = () => {
  ...
  const handleRegister = async () => {
    try {
      ...
      if (response.user) {
        navigate('/'); // Redirige a Home si el registro es exitoso
      } else {
  ...
  return (
    <Box sx={{ maxWidth: 400, margin: "auto", padding: 3, mt: 5 }}>
      ...
      <TextField label="Fecha de Expedición" type="date" value={newUser.fecha_expedicion} onChange={(e) => handleFieldChange("fecha_expedicion", e.target.value)} fullWidth margin="normal" InputLabelProps={{ shrink: true }} />
